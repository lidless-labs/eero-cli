...
class TestFindOneDevice:
    def setup_method(self):
        self.devices = [
            dev(url="https://api/d/42", nickname="Alistair PC", mac="BC:24:11:00:00:42"),
            dev(url="https://api/d/43", nickname="Alistair Phone", mac="AA:BB:CC:00:00:43"),
        ]
    ...
    def test_exact_by_label(self):
        m = _find_one_device(self.devices, "Alistair Phone")
        assert m is not None and m["url"].endswith("/43")
    ...
    def test_ambiguous_returns_none(self, capsys):
        # "Alistair" matches both by fuzzy terms -> must refuse, not guess.
        assert _find_one_device(self.devices, "Alistair") is None
